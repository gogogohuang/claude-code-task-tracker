...
- [ ] push：running append；同 tool done 覆寫；不同 tool append；超過 40 trim
- [ ] 同 at+phase 不重複 append
- [ ] stuck：running+Read+121s → true；119s → false；AskUserQuestion running → false；done → false
- [ ] formatStuckLabel 分秒／分
- [ ] `shouldHandleDeleteKey` 等既有 view 測試若列舉 WatchView 需含 history（刪除鍵仍僅 main）
...
請回覆「核准」或要改的決策（N=40、合併策略、120s、`h`）。  
核准後：plan → TDD 實作（建議新分支，前綴依 `gh api user`）。
