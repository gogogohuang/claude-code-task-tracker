...
- [ ] `isWaitingForUser`：AskUserQuestion／ExitPlanMode + running → true；done → false；Read+running → false
- [ ] `classifyPresence`：三態與 60s 邊界
- [ ] `aggregatePresence`：waiting 蓋 busy 蓋 idle
- [ ] label 前綴穩定
- [ ] gauge：undefined → 無 bar 字串；0%／50%／80%／95% 行為
- [ ] （可選）小測試：waiting key 邊沿去重邏輯若抽成純函式
...
請回覆「核准」或要改的決策（尤其：閒置 60s、label 符號、P2 沿用 1e6）。  
核准後：writing-plans → TDD 實作。
