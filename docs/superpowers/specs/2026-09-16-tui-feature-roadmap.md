...
狀態：進行中（～v0.20 ✅；v0.21 E3+E4 design 已核准；T3 暫緩；C1 ❌）  
基準版本：v0.20.0（功能合入後再依 release 規則 bump）  
...
| M4 | 雙欄 split | 多 session | ✅ v0.20 | |
| T3 | Task 依賴圖 | 進度板 | **暫緩** | v0.21 不做 |
| E3 | 活動句語系 | CLI | ✅ v0.21 | |
| E4 | 複製 id／路徑 | CLI | ✅ v0.21 | |
...
→  v0.21  收尾：E3 + E4（T3 暫緩）· 實作中／完成見 PR
```
