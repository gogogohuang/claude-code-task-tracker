...
目前版本：**v0.12.0**。套件頁：[npm](https://www.npmjs.com/package/claude-code-task-tracker)。
## 運作原理
...
畫面內按 `q` 離開、按 `b` 回專案列表、按 `c` 清除目前 session 暫存（選輕清／重清）、按 `d` 刪除目前 session（需再按一次確認；正在執行時無法刪除）、按 `a` 查看用量建議、↑↓ 在清單裡捲動（一次一頁視窗，不會整份往下刷）。活動列直接顯示那句話，例如 `◐ 正在讀取 src/schema.ts`；結束後變成
